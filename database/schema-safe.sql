-- trAInor Fitness App Database Schema (Safe Version)
...
-- RLS Policies for profiles (drop and recreate to avoid conflicts)
...
DROP POLICY IF EXISTS "Users can view public workout templates" ON workouts;
...
-- Create triggers for updated_at timestamps (drop first to avoid conflicts)
...
-- Trigger to automatically create profile on user signup (drop first to avoid conflicts)
